...
class AudioCDRipper {
  ...
  Future<DiscMetadata?> loadMetadata() async {
    ...
    try {
      ...
      stderr.writeln('Error: ffprobe could not read the disc.');
      return null;
    }
    ...
    if (chapters.isEmpty) {
      stderr.writeln('Error: no tracks found on the disc.');
      return null;
    }
    ...
    if (discId.isEmpty || leadOut <= 0) {
      stderr.writeln('Warning: disc ID or lead-out is invalid '
          '(discId="$discId", leadOut=$leadOut) — metadata lookup may fail.');
    }
  ...
  Future<void> rip(DiscMetadata meta, List<int> trackNumbers) async {
    ...
    if (meta.releaseMbid.isNotEmpty) {
      ...
      if (!await coverFile.exists()) {
        stderr.writeln('Downloading cover art...');
        final bytes = await CoverArt.fetchFront(meta.releaseMbid);
        if (bytes != null) {
          await coverFile.writeAsBytes(bytes);
          stderr.writeln('Cover saved: ${coverFile.path}');
        } else {
          stderr.writeln('No cover art found.');
        }
      }
    }
    ...
    for (final nr in trackNumbers) {
      ...
      if (idx < 0 || idx >= meta.tracks.length) {
        stderr.writeln('Invalid track: $nr');
        continue;
      }
      ...
      stdout.writeln('── Ripping: track $nr → ${outFile.path}');
      ...
      if (exitCode == 0 || await outFile.exists()) {
        ...
        stderr.writeln('   Error ripping track $nr (exit code $exitCode).');
