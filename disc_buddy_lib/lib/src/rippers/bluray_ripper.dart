...
class BlurayRipper implements VideoDiscRipper<BlurayTitle> {
  ...
  BlurayRipper(this.options, {this.onLog, this.onProgress, void Function(Process)? onFfmpegProcess})
      : _ffmpeg = FfmpegRunner(executable: options.ffmpeg, onProcessStarted: onFfmpegProcess);

  static Set<int>? _langsToIndices(List<String> trackLangs, Set<String>? filter) {
    if (filter == null || filter.isEmpty) return null;
    final indices = <int>{};
    for (var i = 0; i < trackLangs.length; i++) {
      if (filter.any((l) => l.toLowerCase() == trackLangs[i].toLowerCase())) {
        indices.add(i);
      }
    }
    return indices.isEmpty ? null : indices; // null = keep all when nothing matches
  }
  ...
  Future<void> _ripMounted(
    ...
    if (isEncrypted) {
      ...
      if (isUhd) {
        _log('');
        _log('   ⚠  4K Ultra HD Blu-ray gedetecteerd (AACS 2.0).', isError: true);
        _log('   libaacs en libbluray ondersteunen alleen AACS 1.0 (standaard Blu-ray).', isError: true);
        _log('');
        return;
      } else if (!hasLibbluray) {
        _log('');
        _log('   ⚠  Encrypted Blu-ray detected and ffmpeg lacks libbluray.', isError: true);
        for (final line in [
          ...
          '      and: ${xdgConfigHome()}/aacs/KEYDB.cfg  (from makemkv or oss-blu-ray)',
        ]) {
          _log(line, isError: true);
        }
        _log('');
      } else if (_findKeydb() == null) {
        _log('');
        _log('   ⚠  Encrypted Blu-ray — libbluray present but no KEYDB.cfg.', isError: true);
        _log('   Place KEYDB.cfg in ${xdgConfigHome()}/aacs/ for AACS decryption.', isError: true);
        _log('');
      }
    }

    if (hasLibbluray) {
      _log('   (libbluray available — AACS decryption active if configured)');
    } else {
      _log('   (libbluray not found — direct M2TS mode)');
    }
    ...
    for (final title in selected) {
      _log('── Languages: playlist ${title.playlist} [${title.durationLabel}]');
  ...
  Future<({String discTitle, List<BlurayTitle> titles})?> _scanMounted(
    ...
    if (!await playlistDir.exists()) {
      _log('Error: BDMV/PLAYLIST not found at $mountPath', isError: true);
      return null;
    }
    ...
    if (titles.isEmpty) {
      _log('Error: no titles found on the Blu-ray.', isError: true);
      return null;
    }
  ...
  Future<void> _ripTitle(
    ...
    _log('── Ripping: playlist ${title.playlist} → ${outFile.path}');
    ...
    final audioIndices = options.audioTrackIndices
        ?? _langsToIndices(langs.audioLangs, options.audioMkvLangs);
    final subIndices = options.subtitleTrackIndices
        ?? _langsToIndices(langs.subLangs, options.subtitleMkvLangs);
    ...
    if (useLibbluray) {
      ...
      if (m2tsFiles.isEmpty) {
        ...
        _log('   No M2TS files found for ${title.playlist}.', isError: true);
        return;
      }
    ...
    var exitCode = await _ffmpeg.run(
      args,
      timeout:          Duration(seconds: dur + 180),
      expectedDuration: title.duration,
      onProgress:       onProgress ?? logProgress,
      stderrCollect:    stderrLines,
    );
    _log('');

    // If libbluray failed due to AACS, fall back to direct M2TS
    var aacsError = false;
    if (exitCode != 0 && useLibbluray) {
      aacsError = stderrLines.any((l) =>
          l.toLowerCase().contains('aacs') ||
          l.contains("can't decrypt") ||
          l.contains('Host key'));
      if (aacsError) {
        await outFile.delete().catchError((_) => File(''));
        _log('   AACS decryption failed — trying direct M2TS fallback...', isError: true);
        final m2tsFiles = clipNames
            .map((c) => p.join(bdmvPath, 'STREAM', '$c.m2ts'))
            .where((f) => File(f).existsSync())
            .toList();
        if (m2tsFiles.isNotEmpty) {
          final m2tsArgs = _buildDirectArgs(
            m2tsFiles:   m2tsFiles,
            audioLangs:  langs.audioLangs,
            subLangs:    langs.subLangs,
            lpcmIdx:     lpcmIdx,
          ...
          _log('   No M2TS files found for fallback.', isError: true);
        }
      }
    }
    ...
    if (exitCode == 0 && fileSize > 0) {
      ...
      if (aacsError) {
        _log('   AACS decryption failed. Update ${xdgConfigHome()}/aacs/KEYDB.cfg or use MakeMKV.', isError: true);
      } else {
        _log('   Error ripping ${title.playlist} (exit code $exitCode).', isError: true);
      }
      await outFile.delete().catchError((_) => File(''));
    }
  }
  ...
  static List<String> _buildLibblurayArgs({
    ...
    Set<int>? audioIndices,
    Set<int>? subIndices,
  }) {
    final selAudio = _resolveIndices(audioIndices, audioLangs.length);
    final selSubs  = _resolveIndices(subIndices,   subLangs.length);
    ...
    final args = [
      ...
      '-map', '0:v',
      if (audioIndices == null) ...[ '-map', '0:a' ]
      else ...[ for (final i in selAudio) ...[ '-map', '0:a:$i' ] ],
      if (subIndices == null) ...[ '-map', '0:s?' ]
      else ...[ for (final i in selSubs) ...[ '-map', '0:s:$i?' ] ],
      '-c:v', 'copy',
    ];

    for (var out = 0; out < selAudio.length; out++) {
      args.addAll(['-c:a:$out', lpcmIdx.contains(selAudio[out]) ? 'flac' : 'copy']);
    }
    if (selAudio.isEmpty) args.addAll(['-c:a', 'copy']);

    args.addAll(['-c:s', 'copy']);
    _addMetadata(args, _pick(audioLangs, selAudio), _pick(subLangs, selSubs),
        _pick(audioTitles, selAudio));
    args.add(outPath);
    return args;
  }
  ...
  /// Returns sorted list of selected indices, clamped to [0, count).
  static List<int> _resolveIndices(Set<int>? indices, int count) {
    if (indices == null) return List.generate(count, (i) => i);
    return (indices.where((i) => i < count).toList()..sort());
  }

  /// Picks elements from [list] at the given [indices], padding with '' if out of range.
  static List<String> _pick(List<String> list, List<int> indices) =>
      [for (final i in indices) i < list.length ? list[i] : ''];
  ...
  static String? _findKeydb() {
    ...
    for (final path in [
      p.join(xdgConfigHome(), 'aacs', 'KEYDB.cfg'),
