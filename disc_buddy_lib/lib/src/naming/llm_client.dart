  ...
  Future<String?> chat(
    ...
    try {
      ...
      if (response.statusCode != 200) {
        _log('   LLM request failed (${response.statusCode}): ${response.body}',
            isError: true);
        return null;
      }
      ...
      if (content == null || content.trim().isEmpty) {
        _log('   LLM: empty/null content (finish_reason=$finish). '
            'Response: ${response.body}', isError: true);
      }
      return content;
    } catch (e) {
      _log('   LLM error: $e', isError: true);
      return null;
    }
  }
