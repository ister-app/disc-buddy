...
class BlurayTitle implements VideoTitle {
  ...
  @override List<String> get audioLangCodes    => audioLangs.map((l) => l.toLowerCase()).toList();
  @override List<String> get subtitleLangCodes => subtitleLangs.map((l) => l.toLowerCase()).toList();
  @override List<String> get audioTrackLabels => List.generate(audioCount, (i) {
    final lang = i < audioLangs.length && audioLangs[i].isNotEmpty
        ? audioLangs[i].toUpperCase()
        : '??';
    final titleStr = i < audioTitles.length ? audioTitles[i] : '';
    final suffix = lpcmAudioIndices.contains(i) ? ' (LPCM)' : '';
    return titleStr.isNotEmpty ? '$lang · $titleStr$suffix' : '$lang$suffix';
  });
  @override List<String> get subtitleTrackLabels => List.generate(subtitleCount, (i) =>
      i < subtitleLangs.length && subtitleLangs[i].isNotEmpty
          ? subtitleLangs[i].toUpperCase()
          : '??');
